...
import { ShoppingCart, Pill, Search, Trash2, Check, X } from 'lucide-react';
...
export default function Carrito() {
  ...
  async function finalizarCompra() {
    ...
    if (!patientId) {
      setMensaje('Debes seleccionar un paciente para finalizar la compra.');
      return;
    }
  ...
  return (
    <section>
      ...
      {mensaje && (
        <div style={{ padding: '12px 16px', background: esExito(mensaje) ? '#e8f8f5' : '#fee', color: esExito(mensaje) ? '#27ae60' : '#c33', borderRadius: 8, marginBottom: 16, fontWeight: 600 }}>
          {mensaje}
        </div>
      )}
